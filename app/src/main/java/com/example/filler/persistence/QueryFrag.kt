...
class QueryFrag : Fragment() {
    ...
    private fun initUI() {
        val adapter = GameSummaryListAdapter(object : GameSummaryClickListener {
            override fun onRowClicked(position: Int) {
                Toast.makeText(activity, "Clicked on row $position", Toast.LENGTH_SHORT).show()
            }

            override fun onRowLongClicked(position: Int) {
                Toast.makeText(activity, "Long clicked on row $position", Toast.LENGTH_SHORT).show()
            }

            override fun onRowDeleteClicked(position: Int) {
                Toast.makeText(activity, "Clicked DELETE on row $position", Toast.LENGTH_SHORT).show()
            }
